...
import { UploadCloud, FileText, Image, HardDrive, Download } from "lucide-react";
...
export default function FileUploader() {
  ...
  const getFileIcon = (filename) => {
    ...
    if (['txt', 'json', 'xml', 'html', 'css', 'js', 'py', 'java', 'cpp', 'c'].includes(ext)) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      alert(`Error processing file: ${err.response?.data?.error || err.message}`);
    } finally {
      setLoading(false);
    }
  };

  const handleDownload = (downloadPath, suggestedFilename) => {
    const link = document.createElement('a');
    link.href = `http://localhost:4000/download/${downloadPath}`;
    link.download = suggestedFilename || downloadPath;
  ...
  const algorithms = [
    {
      ...
      description: "Optimal for text files with repeated characters. Uses variable-length codes.",
      ratio: "~40-60%",
      bestFor: "Text Files",
    },
    {
    ...
    },
    {
      key: "lz77",
      title: "LZ77",
      description: "Dictionary-based compression. Good general-purpose algorithm.",
      ratio: "~30-70%",
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      <h1 className="text-4xl font-bold text-center mb-2 text-gray-800">File Compression Tool</h1>
        ...
        <div className="space-y-6">
          ...
          <div className="border rounded-lg shadow-lg bg-white">
            <div className="bg-blue-600 text-white px-6 py-4 rounded-t-lg">
              ...
              <p className="text-sm opacity-90">Drag and drop files or click to browse</p>
            </div>
            <div className="p-6">
              <div className="border-2 border-dashed border-blue-400 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
                ...
                <p className="text-sm text-gray-500 mb-4">Supports text, image, and binary files</p>
          ...
          <div className="bg-white shadow-lg rounded-lg">
            <div className="bg-blue-600 text-white px-6 py-4 rounded-t-lg">
              ...
              <p className="text-sm opacity-90">Choose your compression algorithm</p>
            </div>
            <div className="p-6 space-y-4">
              {algorithms.map((algo) => (
                <div key={algo.key} className={`border rounded-lg p-4 cursor-pointer transition-colors hover:bg-gray-50 ${algorithm === algo.key ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    <div className="flex-1">
                      <span className="font-semibold text-lg text-gray-800">{algo.title}</span>
                      <p className="text-sm text-gray-600 mt-1">{algo.description}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        <span className="font-medium">Best for:</span> {algo.bestFor} | 
                        <span className="font-medium"> Avg ratio:</span> {algo.ratio}
                      </p>
                    </div>
                  </label>
                </div>
              ...
              <div className="flex gap-2 mt-6">
                <button 
                  ...
                  className={`flex-1 py-3 rounded font-medium transition-colors ${isCompress ? "bg-green-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
                >
                  Compress Files
                </button>
                <button 
                  onClick={() => setIsCompress(false)} 
                  className={`flex-1 py-3 rounded font-medium transition-colors ${!isCompress ? "bg-orange-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
                >
                  Decompress Files
                </button>
              </div>
            ...
            <div className="p-6">
              {result ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-gray-800 capitalize">{result.algorithm}</p>
                    </div>
                    <div>
                      <span className="font-medium text-gray-600">Operation:</span>
                      <p className={`font-medium ${result.isCompress ? 'text-green-600' : 'text-orange-600'}`}>
                    ...
                    {result.originalSize && (
                      <div>
                        ...
                        <p className="text-gray-800">{formatFileSize(result.originalSize)}</p>
                      </div>
                    )}
                    
                    {result.compressedSize && (
                      <div>
                        <span className="font-medium text-gray-600">Compressed Size:</span>
                        <p className="text-gray-800">{formatFileSize(result.compressedSize)}</p>
                      </div>
                    )}
                    
                    {result.ratio && (
                      <div>
                        <span className="font-medium text-gray-600">Compression Ratio:</span>
                        <p className="text-gray-800 font-medium">{result.ratio}</p>
                      </div>
                    )}
                  ...
                  <button
                    onClick={() => handleDownload(result.downloadPath, result.originalFilename)}
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">Upload and process a file to see statistics</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="p-6">
              {history.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">Your recent operations will appear here</p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              {item.isCompress ? "Compressed" : "Decompressed"} with {item.algorithm.toUpperCase()} 
